...
const Authorization = () => {
  ...
  return (
    <div className={styles.AuthContainer}>
      {cookies["token"] && (
        <Link to={`/Account/${decodedToken.uid}`}>
          <HiOutlineUserCircle size={50} className={styles.userIconContainer} />
        </Link>
      )}
      {cookies["token"] ? (
        <LogOutButton />
        ...
        <div className={styles.Authorization_container}>
          <div className={styles.Authorization} onClick={triggerLogInModal}>
            Log in / Sign up
          </div>
        </div>
      )}
